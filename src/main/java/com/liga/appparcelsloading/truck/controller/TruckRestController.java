package com.liga.appparcelsloading.truck.controller;
...
public class TruckRestController {
     ...
     */
    @PostMapping("/load")
    public ResponseEntity<List<TruckDto>> loadTrucks(
            @RequestParam String algorithmType,
            @RequestParam String heights,
            @RequestParam String weights) {
        Optional<List<TruckDto>> loadedTrucks = truckRestService.load(algorithmType, heights, weights);
        return loadedTrucks
    ...
    public ResponseEntity<List<TruckDto>> loadTrucksByName(
            @RequestParam String algorithmType,
            @RequestParam String nameParcels,
            @RequestParam String heights,
            @RequestParam String weights) {
        Optional<List<TruckDto>> loadedTrucks = truckRestService.loadByName(algorithmType, nameParcels, heights, weights);
        return loadedTrucks
                .map(ResponseEntity::ok)
                .orElseGet(() -> ResponseEntity.badRequest().body(null));
    }
